 ...
 *
 * Pagination wraps (last -> first, first -> last) and auto-advances
 * on a timer by default — see `autoPlay`/`autoPlayIntervalMs`.
 * ---------------------------------------------------------------- */
...
export interface ScrollReelTestimonialsProps {
  ...
  /** Auto-advance to the next testimonial on a timer (default true) */
  autoPlay?: boolean;
  /** Time between auto-advances, in ms (default 5000) */
...
export function ScrollReelTestimonials({
  ...
  // Auto-advance on a timer. Restarts whenever `index` changes (manual
  // or automatic) so a click doesn't get immediately followed by an
  // auto-advance a moment later — every advance gets the full interval.
  React.useEffect(() => {
    if (!autoPlay || count < 2) return;
    const id = setInterval(() => paginate(1), autoPlayIntervalMs);
    return () => clearInterval(id);
  }, [autoPlay, autoPlayIntervalMs, index, count, paginate]);

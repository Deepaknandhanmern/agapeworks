...
export async function GET() {
  ...
  const leads = await db.viviraDownloadLead.findMany({ orderBy: { createdAt: "desc" } });
  const rows = [
    "name,email,downloaded_at",
    ...leads.map((l) => `${l.name},${l.email},${l.createdAt.toISOString()}`),
  ];

  return new NextResponse(rows.join("\n"), {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": "attachment; filename=vivira-downloads.csv",
    },
  });
}
